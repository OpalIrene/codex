...
def format_price(value: float, mode: str = "Auto (2/4/6)") -> str:
    try:
        v = float(value)
    except Exception:
        return str(value)
...
if not coins:
    ...
    with top_section:
        ...
        if featured_coins:
            ...
            featured_cols = st.columns(len(featured_coins))
            for col, coin in zip(featured_cols, featured_coins):
                logo_url = coin.get("image", "")
                symbol = coin.get("symbol", "").upper()
                price_str = format_price(coin.get("current_price", 0), price_format)
                delta = coin.get("price_change_percentage_24h", 0) or 0
                card_html = f"""
                <div title="{coin.get('name', '')}" style="background:#0f1724;padding:12px;border-radius:16px;text-align:center;color:#e2e8f0;min-height:170px;">
                  <div style="display:flex;align-items:center;justify-content:center;margin-bottom:8px;">
                    <img src=\"{logo_url}\" width=34 height=34 style=\"border-radius:50%;margin-right:8px;\" />
                    <div style=\"font-weight:700;font-size:15px;\">{symbol}</div>
                  </div>
                  <div style="font-size:18px;font-weight:700;margin-top:4px;">{price_str}</div>
                  <div style="font-size:13px;color:{'lime' if delta>=0 else 'salmon'};font-weight:600;margin-top:6px;">{delta:.2f}%</div>
                  <div style="margin-top:8px;font-size:12px;color:#94a3b8;">{coin.get('name','')}</div>
                </div>
                """
        ...
        if price_col_g in gainers_df.columns:
            gainers_df[price_col_g] = gainers_df[price_col_g].apply(lambda x: format_price(x, price_format))
    ...
    with market_section:
        ...
        if featured_coins:
            ...
            if price_col_feat in feat_df.columns:
                feat_df[price_col_feat] = feat_df[price_col_feat].apply(lambda x: format_price(x, price_format))
        ...
        if price_col in display_df.columns:
            display_df[price_col] = display_df[price_col].apply(lambda x: format_price(x, price_format))
